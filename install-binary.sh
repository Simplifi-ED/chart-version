...
PROJECT_NAME="chart-version"
PROJECT_GH="muandane/$PROJECT_NAME"
...
installFile() {
  ...
  HELM_TMP_BIN="$HELM_TMP/chart-version"
  if [ "${OS}" = "windows" ]; then
...
exit_trap() {
  ...
  if [ "$result" != "0" ]; then
    ...
    printf '\tFor support, go to https://github.com/muandane/chart-version.\n'
  fi
  exit $result
}
